...
pub fn get_balance_requirement(connection: &RpcClient) -> Result<u64> {
    let account_fee =
        connection.get_minimum_balance_for_rent_exemption(utils::get_result_data_size()?)?;
...
/// smart contract we need some way to store the result of executing
/// our bytecode to the contract. To do this we create a result
...
/// deems fit.
///
/// The result account has a [derived
...
/// like to find the result account again later.
pub fn create_result_account(
    player: &Keypair,
    program: &Keypair,
    connection: &RpcClient,
) -> Result<()> {
    let result_pubkey = utils::get_result_public_key(&player.pubkey(), &program.pubkey())?;

    if let Err(_) = connection.get_account(&result_pubkey) {
        let lamport_requirement =
            connection.get_minimum_balance_for_rent_exemption(utils::get_result_data_size()?)?;

        // This instruction creates an account with the key
        // "result_pubkey". The created account is owned by the
        ...
        // generated by utils::get_result_public_key. We do this as
        ...
        let instruction = solana_sdk::system_instruction::create_account_with_seed(
            ...
            &result_pubkey,
            &player.pubkey(),
            utils::get_result_seed(),
            lamport_requirement,
            utils::get_result_data_size()? as u64,
            &program.pubkey(),
        );
...
/// previously generated result account. The program will use that
/// passed in address to update the result account data after
/// verifying that it owns the account that we have passed in.
pub fn execute_program(
    ...
    let instruction = Instruction::new_with_bytes(
        program.pubkey(),
        instruction_data,
        vec![AccountMeta::new(result_pubkey, false)],
    );
    let message = Message::new(&[instruction], Some(&player.pubkey()));
...
/// been run.
pub fn count_greetings(player: &Keypair, program: &Keypair, connection: &RpcClient) -> Result<i32> {
    let result_pubkey = utils::get_result_public_key(&player.pubkey(), &program.pubkey())?;
    let result_account = connection.get_account(&result_pubkey)?;
    Ok(utils::get_result(&result_account.data)?)
}
