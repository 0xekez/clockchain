...
pub fn get_result_seed() -> &'static str {
    "results"
}

/// Derives and returns the greeting account public key for a given
/// PLAYER, PROGRAM combination.
pub fn get_result_public_key(player: &Pubkey, program: &Pubkey) -> Result<Pubkey> {
...
pub fn get_result_data_size() -> Result<usize> {
    let encoded = ResultSchema { result: 0 }
        .try_to_vec()
...
/// greeting counter.
pub fn get_result(data: &[u8]) -> Result<i32> {
    let decoded = ResultSchema::try_from_slice(data).map_err(|e| Error::SerializationError(e))?;
    Ok(decoded.result)
}
